...
const PROJECTS = [
  ...
  {
    name: "SingLoRA-CLIP",
    desc: "Ultra-efficient VLM adaptation: replacing LoRA's dual-matrix update with a single symmetric matrix (AAᵀ) — 50% fewer trainable parameters, SOTA few-shot across 8 datasets.",
    img: "/img/deepmirror.png",
...
function Project({ p }: { p: (typeof PROJECTS)[number] }) {
  ...
  return (
    <div ref={ref} className={inView ? "animate-fade-in-up" : "opacity-0"}>
      <div className="ml-20 md:ml-28 mb-6 flex items-start justify-between gap-6">
        <div>
      ...
      <a href={p.url} target="_blank" rel="noreferrer">
        <img src={p.img} alt={p.name} className="w-full rounded-2xl shadow-lg object-cover max-h-[480px]" />
      </a>
    </div>
  );
}
...
export function ProjectsSection() {
  return (
    <section id="work" className="max-w-[1200px] mx-auto px-6 py-12">
      <div className="ml-20 md:ml-28 mb-10">
